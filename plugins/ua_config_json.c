...
loadCertificateFile(const char *const path) {
    ...
    UA_fseek(fp, 0, SEEK_END);
    fileContents.length = (size_t)UA_ftell(fp);
    fileContents.data = (UA_Byte *)UA_malloc(fileContents.length * sizeof(UA_Byte));
    if(fileContents.data) {
        UA_fseek(fp, 0, SEEK_SET);
        size_t read = UA_fread(fileContents.data, sizeof(UA_Byte), fileContents.length, fp);
